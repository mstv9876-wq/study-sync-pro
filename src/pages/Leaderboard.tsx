          ...
          <div className="space-y-3">
            {displayUsers.map((u, i) => (
              <motion.div
                ...
                className={`bg-card rounded-xl p-4 flex items-center gap-4 border border-border shadow-sm ${i < 3 ? "ring-1 ring-yellow-200" : ""}`}
              >
                <span className="text-2xl w-10 text-center">
                  {i < 3 ? medals[i] : <span className="text-muted-foreground text-lg">#{i + 1}</span>}
                </span>
                <div className="flex-1">
                  <span className="font-semibold text-foreground">{u.display_name}</span>
                ...
                <div className="text-right">
                  <div className="text-lg font-bold text-brand">{u.focus_score || 0}</div>
